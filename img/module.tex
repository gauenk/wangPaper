...
\resizebox{42.5em}{85em}{%
  \begin{centering}
    \begin{tikzpicture}[rotate=90]
      ...
      \filldraw[color=red!60, fill=red!5, very thick,](B) rectangle +(2,3) node {};
      \draw (B)+(1,1.5) node {Module};

      \draw[ultra thick, ->] (5,2) -> (6,2) node [pos=0.5,rotate=90,above] (TextNode) {$\mathbf{{}^{^{L+1}}\!z}$};
      \draw[ultra thick, ->] (6,1) -> (5,1) node [pos=0.5,rotate=90,above] (TextNode) {$\mathbf{{}^{^{L+1}}\!\delta}$};
      \draw[ultra thick, ->] (2,2) -> (3,2) node [pos=0.5,rotate=90,above] (TextNode) {$\mathbf{{}^{^{L}}\!z}$};
      \draw[ultra thick, ->] (3,1) -> (2,1) node [pos=0.5,rotate=90,above] (TextNode) {$\mathbf{{}^{^{L}}\!\delta}$};
      \draw[ultra thick, ->] (4,0) -> (4,-1) node [pos=0.5,above] (TextNode) {$\mathbf{\frac{\partial C}{\partial \Theta^{L}}}$};
